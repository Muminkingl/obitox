...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const currentUsage = {
      ...
      successfulRequests: activeDailyUsage.reduce((sum, day) => sum + (day.total_requests || 0), 0), // Fallback to total since successful_requests is removed
      failedRequests: 0, // Fallback since failed_requests is removed
      totalFileSize: 0, // Fallback since total_file_size is removed
...
function generateMonthlyDataFromDaily(dailyUsage: any[], startDate: Date, endDate: Date) {
  ...
  dailyUsage.forEach(day => {
    ...
    if (monthlyMap.has(monthKey)) {
      ...
      monthData.bandwidth += 0; // Convert to MB, defaulting to 0 as total_file_size is removed
      monthData.apiCalls += day.total_requests || 0;
      monthData.successfulRequests += day.total_requests || 0; // Using total as fallback
    }
  });
...
function formatProviderBreakdown(providerStats: any[]) {
  ...
  return filteredStats.map(provider => ({
    ...
    totalFileSize: 0, // Fallback since total_file_size is removed
    averageFileSize: 0, // Fallback since average_file_size is removed
...
function extractFileTypeBreakdown(providerStats: any[]) {
  ...
  filteredStats.forEach(provider => {
    // Fallback: file_type_counts column was removed from provider_usage
    // We optionally could look for it elsewhere, but for now we skip plotting actual file types
    // Since we can't find the column, we safely ignore
  });
